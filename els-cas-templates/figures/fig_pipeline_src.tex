...
% ---------------- row 4: SC3R ----------------
\node[sc3r, below=20mm of loio.south west, anchor=north west] (sc3rsrc) {source-class normals\\(matched condition)};
\node[sc3r, right=of sc3rsrc] (sc3rnorm) {support-normalized\\residuals};
\node[sc3r, right=of sc3rnorm] (sc3rthr) {source-validated\\threshold $\tau(\alpha)$};
\node[outbox, right=of sc3rthr] (alarmout) {alarms below the floor:\\$\mathrm{FAR}\approx\alpha$, no target\\anomaly labels};
...
\draw[arr, dashed] ($(floor.south east)-(3mm,0)$) |- ($(alarmout.north)+(0,5mm)$)
  node[note, pos=0.72, above]{$\alpha\ge\tfrac{1}{k+1}$: target-only LOIO (safe anchor)} -- (alarmout.north);
...
\node[lanelabel, text=red!60!black!80] at (sc3rframe.north west) {SC3R --- cross-category, below the floor};

% ---------------- diagnostics strip ----------------
\node[note, anchor=north west, yshift=-1.5mm] at (sc3rframe.south west)
  {audit: exact discrete uniformity (Q--Q) $\cdot$ FAR at fixed $\alpha$ $\cdot$ risk--coverage $\cdot$ ECE secondary (prevalence-sensitive)};
