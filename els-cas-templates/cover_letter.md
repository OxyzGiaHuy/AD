...
2. **SC3R, a source-validated cross-category thresholding mechanism** that restores validity below the floor using only structure the learning system already has (normal examples of other categories). Against a pre-registered decision gate it tracks nominal false-alarm rates almost exactly on all 15 MVTec classes, replicates on all 12 VisA classes, and transfers conservatively across datasets, with hierarchical bootstrap intervals excluding zero for every corruption condition.
