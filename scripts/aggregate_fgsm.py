...
def detailed_rows(outputs_dir: Path, robustness_dir: Path) -> list[dict]:
    ...
    rows = []
    for metrics_path in robustness_dir.glob("calib_subspace_head_mvtec_*_fgsm_eps8_255/metrics.json"):
        match = RUN_RE.match(metrics_path.parent.name)
        if not match:
            continue
        ...
        rows.append(
            {
                ...
                "epsilon": attack_metrics.get("epsilon", "8/255"),
    ...
    return sorted(rows, key=lambda r: (r["k_shot"], r["class"], r["seed"]))


def summarize(rows: list[dict]) -> list[dict]:
    grouped = defaultdict(list)
    for row in rows:
        grouped[row["k_shot"]].append(row)
    out = []
    for k, group_rows in sorted(grouped.items()):
        row = {"k_shot": k, "n": len(group_rows)}
        for metric in [
            "clean_auroc",
            "fgsm_auroc",
            "auroc_drop_abs",
        ...
        out.append(
            {
                ...
                "clean_ap": f"{fmt(row['clean_ap_mean'])} +/- {fmt(row['clean_ap_std'])}",
                "fgsm_ap": f"{fmt(row['fgsm_ap_mean'])} +/- {fmt(row['fgsm_ap_std'])}",
...
def main() -> int:
    ...
    if not rows:
        print("No FGSM runs found")
        return 1
    out_dir = Path(args.out_dir)
    summary = summarize(rows)
    write_csv(out_dir / "mvtec_fgsm_detailed.csv", rows)
    write_csv(out_dir / "mvtec_fgsm_summary.csv", summary)
    write_markdown(out_dir / "mvtec_fgsm_summary.md", compact(summary))
    print(f"runs={len(rows)}")
    print(out_dir / "mvtec_fgsm_summary.csv")
    print(out_dir / "mvtec_fgsm_summary.md")
    return 0
