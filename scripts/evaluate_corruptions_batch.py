...
def evaluate_one_corruption(
    *,
    ...
    metrics.update({"k_shot": k, "seed": seed, "corruption": corruption, "num_images": len(eval_corrupt)})

    run_name = f"{experiment_cfg.get('name', 'experiment')}_{model_cfg.get('variant', 'model')}_k{k}_seed{seed}_{corruption}"
...
def main() -> int:
    ...
    parser.add_argument("--corruptions", nargs="+", choices=sorted(CORRUPTION_FUNCS), default=sorted(CORRUPTION_FUNCS))
    ...
    for corruption in args.corruptions:
        run_name = f"{experiment_cfg.get('name', 'experiment')}_{model_cfg.get('variant', 'model')}_k{k}_seed{seed}_{corruption}"
        metrics_path = Path(experiment_cfg.get("output_dir", "outputs")) / "robustness" / run_name / "metrics.json"
        if metrics_path.exists():
            print(f"SKIP {metrics_path}", flush=True)
        else:
            pending_corruptions.append(corruption)
    if not pending_corruptions:
        return 0
    ...
    corrupt_records_by_name = {}
    corrupt_batch_by_name = {}
    for corruption in pending_corruptions:
        tmp_dir = Path(args.tmp_root) / dataset_cfg.get("name", "dataset") / class_key / f"seed{seed}" / corruption
        eval_corrupt = corrupt_records(eval_clean, corruption, tmp_dir, seed=seed, max_images=args.max_images)
        corrupt_records_by_name[corruption] = eval_corrupt
        corrupt_cache_name = f"{dataset_name}_corrupt_{class_key}_{corruption}_{backbone_name}_seed{seed}"
        corrupt_cache_seed = 0 if backbone_name.startswith("dinov2") else seed
        corrupt_batch_by_name[corruption] = load_feature_cache_if_present(
            eval_corrupt,
            cache_dir,
            corrupt_cache_name,
            seed,
    ...
    if support_batch is None or any(batch is None for batch in corrupt_batch_by_name.values()):
        ...
        for corruption, batch in list(corrupt_batch_by_name.items()):
            if batch is not None:
                continue
            corrupt_cache_name = f"{dataset_name}_corrupt_{class_key}_{corruption}_{backbone_name}_seed{seed}"
            corrupt_cache_seed = 0 if backbone_name.startswith("dinov2") else seed
            corrupt_batch_by_name[corruption] = encode_with_cache(
                backbone,
                corrupt_records_by_name[corruption],
                cache_dir,
                corrupt_cache_name,
                seed,
                backbone_name,
    ...
    for corruption in pending_corruptions:
        out_dir = evaluate_one_corruption(
            config=config,
            model=model,
            support_features=support_features,
            support_scores=support_scores,
            eval_corrupt=corrupt_records_by_name[corruption],
            corrupt_features=corrupt_batch_by_name[corruption].patch_features,
            corruption=corruption,
